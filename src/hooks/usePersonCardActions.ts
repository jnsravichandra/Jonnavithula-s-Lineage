...
function usePersonCardActions() {
  ...
  const getMember = useCallback(async (memberId: string) => {
    const currentMember: Member | null = await MemberService.getMemberById(
      memberId
    );
    if (currentMember) {
  ...
  const handleDelete = useCallback(
    async (member: TreeNode): Promise<DeleteResult> => {
      // console.log(`Delete ${member.member_id}`);
      try {
        await MemberService.deleteMember(member.member_id);
        const successMessage = `Member "${member.first_name} ${
          member.last_name || ""
